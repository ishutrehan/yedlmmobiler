	...
	public function register()
	{
		...
		if(isset($_POST) && !empty($_POST)){
			...
			if(!isset($_POST['role']) || empty($_POST['role'])){
				$errors['error'] = true;
				$errors['message']['role'] = ['Role should not be empty!'];
			}
			//validate name
			if(!isset($_POST['name']) || empty($_POST['name'])){
				$errors['error'] = true;
				$errors['message']['name'] = ['Name should not be empty!'];
			}
			//validate email address
			if(!isset($_POST['email']) || empty($_POST['email'])){
				$errors['error'] = true;
				$errors['message']['email'] = ['Email address should not be empty!'];
			}else{
				...
				if($user_exist){
					$errors['error'] = true;
					$errors['message']['email_exists'] = "Email address already exists.";
				}
			}
			//validate password
			if(!isset($_POST['password']) || empty($_POST['password'])){
				$errors['error'] = true;
				$errors['message']['password'] = ['Password should not be empty!'];
			}
			//validate phone
			if(!isset($_POST['phone']) || empty($_POST['phone'])){
				$errors['error'] = true;
				$errors['message']['phone'] = ['Phone should not be empty!'];
			}
			...
			if(empty($errors)){
				...
				if($user){
					if($user[0]['status'] == 'approved'){						
						...
						echo json_encode(['error' => true, 'message'=> 'Your profile is not approved.']);
					}
				}else{
